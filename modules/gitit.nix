...
let
  ...
  gititWithPkgs = hsPkgs: extras:
    hsPkgs.ghcWithPackages (self: with self;
      [
        (pkgs.haskell.lib.appendPatches gitit [ ./gitit-dyntoo.patch ])
      ] ++ (extras self)
    );
...
video/x-ms-wmx  wmx
  ...
  '' + lib.optionalString (cfg.githubOrg != null) ''
  ...
  config = mkIf cfg.enable {
    ...
    in {
      ...
      with cfg; ''
        ...
        cp -Lr --no-preserve=all ${./../gitit/data}/* ${homeDir}/data
        find ${homeDir}/data -type d -exec chmod 0750 {} +
        find ${homeDir}/data -type f -exec chmod 0640 {} +
